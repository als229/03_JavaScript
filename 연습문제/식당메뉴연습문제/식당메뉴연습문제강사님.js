...
resName.addEventListener("click", () => {
  ...
  resName.classList.add("res-name-hidden"); // 숨기기 클래스 추가
  resNameInput.classList.remove("res-name-hidden"); // 숨기기 클래스 제거
  resNameInput.focus();

  resNameInput.value = resName.innerText;
});
...
resNameInput.addEventListener("blur", () => {
  ...
  resNameInput.classList.add("res-name-hidden"); // 숨기기 클래스 추가
  resName.classList.remove("res-name-hidden"); // 숨기기 클래스 제거
...
updateBtn.addEventListener("click", () => {
  ...
  menus.forEach(menu => {
    ...
    const menuName  = menu.children[0].innerText;
    const menuPrice = menu.children[1].innerText.replace('원','');

    // 5) .menu 내부에 추가될 input요소를 모아둔 객체를 모아둔 JS 객체 얻어오기
...
function createMenuContent(){
  ...
  // 메뉴 가격 input 생성
  const menuPriceInput = newEl("input",
