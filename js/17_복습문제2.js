...
// 향상된 for 문 : 0 ~ 끝 까지 모든 요소를 순차 접근하는 용도의 for 문
for(let input of colorInputs){
  // input = colorInputs[i] , i는 0부터 끝까지 1씩 증가
...
for(let i = 0; i < radioBtns.length; i++){
  radioBtns[i].addEventListener("change", function(e){
    ...
    // radio 버튼은 체크 X = > 체크 O 로 변할때 change 이벤트가 감지된다.
    ...
    /* 
      ...
      - for 문이 반복될 때 마다 배열 내 요소를 0번부터 하나씩 꺼내서 of 앞에 변수에 대입하는 for 문
    */
    for(let box of boxes){
      // for 문이 반복 될 때 마다
      // boxes[0] ~ boxes[4]까지
      // 1개씩 차례대로 box 변수값에 대입
