/* 
  [템플릿 문자열]
  - `(백틱)을 이용하여 문자열 + 변수/값 혼합 작성을 쉽게 할 수 있게 해주는 방법
  - 문자열 전체를 ``(백틱)으로 감싸고
    변수/값이 들어가야 되는 부분에는 ${변수명/값} 형태로 작성

  ex)
  const num = 10;

  console.log(`num 에 저장된 값은 ${num} 입니다.`);
...
function oddEvenCheck(){
  ...
  // 짝수 == 2 의 배수 == 2로 나눴을 때 나머지가 0인 수
  console.log("짝수 맞음? :", (val % 2 === 0) );
  // 홀수 == 2 의 배수 -1, +1 == 2로 나눴을때 나머지가 1인 수
...
function compoundCheck(){
  ...
  num -= 60; // num = num - 60;
  console.log(`num : ${num}`); // 60
  
  num *= 3; // num = num * 3;
  console.log(`num : ${num}`); // 180
...
function andCheck(){
  ...
  console.log(`104는 100 이상의 숫자이면서 짝수인가? ${bool1}`); // T
  ...
  console.log(`13은 1부터 10 사이의 숫자가 맞는가? ${bool3}`); // T
}
...
function orCheck(){
  ...
  console.log(`4는 10을 초과하거나 짝수인가? ${bool1}`); // T
  ...
  console.log(`나이가 18세 미만 이거나, 이름이 "홍길동"인가? ${bool2}`); // T
  ...
  console.log(`국어 또는 영어 점수가 40점 미만이거나 평균이 60점 미만인가? ${bool3}`); // F
}
